...
const Folder = () => {
  ...
  const saveFoldersToExternalApi = async () => {
    try {
      const response = await axios.post(
        "http://127.0.0.1:3000/folders/create",
        {
          unique_id: uuidv4(),
  ...
  return (
    <div>
      <center>
        <div
          className="bg-white w-[90vw] h-[75vh]  rounded-lg border-2
                       border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] px-20 py-12 overflow-y-auto"
        >
          <div className="bg-yellow-400 h-16 absolute w-16 rounded-full border-0 top-8 left-[10%] mt-4 -rotate-12  border-black shadow-[8px_8px_0px_rgba(0,0,0,1)] over"></div>
          <center>
